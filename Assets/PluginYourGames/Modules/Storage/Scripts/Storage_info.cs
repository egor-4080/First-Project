...
#else
            [Tooltip(
                "On/Off local saves. When cloud saves are enabled, local and cloud saves are synchronized! Local saves can be useful when replacing cloud-based ones for a specific platform. For the Yandex Games platform, it is better to turn off local saves and use cloud ones.")]
#endif
...
#else
            [Tooltip(
                "Flush — determines the order in which data is sent. If the value is «true», the data will be sent to the server immediately; «false» (default value) — the request to send data will be queued. (It is recommended to leave this option disabled)")]
#endif
