    ...
    public class FontMasseInstallEditorWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (GUILayout.Button("Search for all objects on the scene by type LanguageYG", GUILayout.Height(30)))
            {
                ...
                foreach (LanguageYG obj in FindObjectsByType<LanguageYG>(FindObjectsInactive.Include,
            ...
            if (objectsTranlate.Count > 0)
            {
                ...
                if (GUILayout.Button("Put a standard font on all selected texts", GUILayout.Height(30)))
                {
                    ...
                    foreach (GameObject obj in objectsTranlate)
                    {
                        ...
                        if (scr.info.fonts.defaultFont.Length >= scr.fontNumber + 1 &&
                            scr.info.fonts.defaultFont[scr.fontNumber])
                        {
                        ...
                        {
                            Debug.LogError(
                                "The standard font is not specified! Specify it in the InfoYG plugin settings.",
                                scr.gameObject);
                        }
                    }
                    ...
                    {
                        Debug.Log(
                            $"Fonts have been partially replaced! Replaced {completeObjCoint} fonts from {objectsTranlate.Count}");
                    }
                }
            }
            ...
            if (objectsTranlate.Count > 10 && position.height < objectsTranlate.Count * 20.6f + 160)
                scrollPosition = GUILayout.BeginScrollView(scrollPosition, false, true,
                    GUILayout.Height(position.height - 160));

            for (int i = 0; i < objectsTranlate.Count; i++)
            {
                objectsTranlate[i] = (GameObject)EditorGUILayout.ObjectField($"{i + 1}. {objectsTranlate[i].name}",
                    objectsTranlate[i], typeof(GameObject), false);
            }
