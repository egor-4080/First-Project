...
namespace YG.Insides
{
    public static partial class Langs
    {
        ...
        public const string t_updateLBMethod =
 ...
 "Продвинутая таблица. Поддерживает подгрузку авата и конвертацию рекордов в тип Time. Подгружает все данные в отдельные элементы интерфейса.";
        ...
        public const string t_playerDataPrefab =
 "Префаб отображаемых данных игрока (объект со компонентом LBPlayerDataYG)";
        public const string t_playerPhoto =
 "Размер подгружаемых изображений игроков. NonePhoto = не подгружать изображение";
        public const string t_isHiddenPlayerPhoto =
 "Использовать кастомный спрайт для отображения аваторок скрытых пользователей";
        public const string t_timeTypeConvert = "Конвертация полученных рекордов в Time тип";
        public const string t_decimalSize =
 "Размер десятичной части счёта (при использовании Time type).\n  Например:\n  0 = 00:00\n  1 = 00:00.0\n  2 = 00:00.00\n  3 = 00:00.000\nВы можете проверить это в Unity не прибегая к тестированию в сборке.";
        ...
        public const string t_updateLBMethod =
            "When should the leaderboard be updated?\nStart - Update in the Start method.\nOnEnable - Update each time the object is activated (in the OnEnable method)\nDoNotUpdate - Do not update the leaderboard using this script (it is assumed that you will run the 'UpdateLB' update method yourself when you need it.";

        public const string t_entriesText =
            "Drag the Text component to write the table description if you have not selected the advanced table";

        public const string t_advanced =
            "An advanced table. Supports uploading an avatar and converting records to the Time type. Loads all data into separate interface elements.";

        public const string t_rootSpawnPlayersData = "The parent object for spawning 'playerDataPrefab' objects in it";

        public const string t_playerDataPrefab =
            "Prefab of the player's displayed data (an object with the LBPlayerDataYG component)";

        public const string t_playerPhoto =
            "The size of the uploaded images of the players. NonePhoto = do not upload an image";
