...
#else
            [Tooltip(
                "Should I show ads when loading the game? (The first advertisement when opening the game). In Unity Editor, the first advertisement will not be simulated - so as not to interfere.")]
#endif
...
#else
            [Tooltip(
                "When the timer is over and the ad can be shown, a request is sent to open the ad. The platform may refuse the request. In this case, by default, new requests will be sent during the next execution of the ad display method until the ad is successfully displayed.\n\nBut you can set a timer after a failed request. For example, set a timer for 10 seconds, and if the ad was not shown, a timer will be set for the next ad request.")]
#endif
