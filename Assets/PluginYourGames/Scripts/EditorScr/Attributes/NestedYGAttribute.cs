        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            if (ShouldDisplay(property))
            {
                ...
                if (attr.drawLine)
                {
                    ...
                    Handles.DrawLine(new Vector3(position.x + offset, position.y),
                        new Vector3(position.x + offset, position.y + angleSize));
                    Handles.DrawLine(new Vector3(position.x + offset, position.y + angleSize),
                        new Vector3(position.x + offset + angleSize, position.y + angleSize));
                }
